...
class BaseStorage(object, metaclass=abc.ABCMeta):
    ...
    **Data persistence**
    ...
    def get_worst_trial(self, study_id: int) -> FrozenTrial:
        """Return the trial with the worst value in a study.
        ...
        Returns:
            The trial with the worst objective value among all finished trials in the study.
        ...
        if direction == StudyDirection.MAXIMIZE:
            worst_trial = min(all_trials, key=lambda t: cast(float, t.value))
        else:
            worst_trial = max(all_trials, key=lambda t: cast(float, t.value))

        return worst_trial

    def get_best_trial_from_last_n(self, study_id: int, last_n: int) -> FrozenTrial:
        """Return the trial with the best value in a sub-set of study.
        ...
        Args:
            study_id:
                ID of the study.
            last_n:
                number of trials from tail to select the best one from

        Returns:
            The trial with the best objective value among sub-set of finished trials in the study.
        ...
        """
        all_trials = self.get_all_trials(study_id, deepcopy=False)
        all_trials = [t for t in all_trials[-last_n:] if t.state is TrialState.COMPLETE]
