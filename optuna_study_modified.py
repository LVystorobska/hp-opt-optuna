...
class Study:
    ...
    @property
    def worst_trial(self) -> FrozenTrial:
        """Return the worst trial in the study.

        Returns:
            A :class:`~optuna.FrozenTrial` object of the worst trial.
        ...
        if self._is_multi_objective():
            raise RuntimeError(
                "A single worst trial cannot be retrieved from a multi-objective study. Consider "
                "using Study.best_trials to retrieve a list containing the worst trials."
            )

        return copy.deepcopy(self._storage.get_worst_trial(self._study_id))
    ...
    def best_trial_from_last_n(self, last_n: int = 5) -> FrozenTrial:
        ...
        return copy.deepcopy(self._storage.get_best_trial_from_last_n(self._study_id, last_n=last_n))
